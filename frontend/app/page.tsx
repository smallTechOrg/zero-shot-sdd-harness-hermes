...
// Labelled, clearly non-functional stubs (Phase 2+). Never mistaken for bugs.
const STUBS = [
  "Chords drill",
  "Rhythm tapping",
...
export default function Page() {
  ...
  const startDrill = useCallback(async () => {
    ...
    setRevealed(null);
    try {
      const data = await jpost<{ drill_id: string; teaching: Teaching; exercise: Exercise }>(
        `${API}/exercises/start`,
        { student_id: studentId, clefs }
      );
      setDrillId(data.drill_id);
      setTeaching(data.teaching);
      setExercise(data.exercise);
      setStatus("ready");
      // auto-play the first note so the learner hears it
      setTimeout(() => playNote(data.exercise.id), 250);
    } catch (e: any) {
      setError(e.message || "Failed to start drill");
      setStatus("idle");
    }
  }, [studentId, clefs, playNote]);
  ...
  const nextNote = useCallback(async () => {
    ...
    try {
      ...
      setExercise(ex);
      setStatus("ready");
      setTimeout(() => playNote(ex.id), 200);
    } catch (e: any) {
      setError(e.message || "Failed to load next note");
      setStatus("ready");
    }
  }, [drillId, studentId, playNote]);
  ...
  const submit = useCallback(
    async (answer: string) => {
      ...
      try {
        ...
        if (res.correct) {
          setFeedback({ kind: "correct", msg: `Correct! That's ${res.computed_name}.` });
  ...
  const reveal = useCallback(
    async (name: string) => {
      setRevealed(name);
      speak(exercise!.id, `The note is ${name}.`);
      setTimeout(() => nextNote(), 1600);
    },
    [exercise, speak, nextNote]
  );

  // When a drill is active, pre-load upcoming notes via SSE (streaming).
  useEffect(() => {
    if (!drillId) return;
    ...
    <main className="mx-auto max-w-3xl px-4 py-8">
      <header className="mb-6">
        ...
        <p className="mt-1 text-slate-500">
          Learn to read staff notation — one note at a time.
        </p>
      </header>
      ...
      {status === "idle" && !exercise && (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center text-slate-400">
          Click <span className="font-semibold text-slate-600">Start drill</span> to
          begin. A note will appear on the staff and play its sound.
        </div>
      )}
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <section>
            ...
            <p className="mt-2 text-center text-sm text-slate-500">
              {exercise.clef} clef · which note is this?
            </p>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              {exercise.options.map((opt) => (
                <button
                  ...
                  onClick={() => submit(opt)}
                  disabled={!!revealed}
                  className="rounded-lg border border-slate-300 bg-white px-3 py-3 text-lg font-semibold text-slate-800 transition hover:border-emerald-500 hover:bg-emerald-50 disabled:opacity-50"
                >
                  {opt}
                </button>
              ))}
            ...
            {showReasoning && teaching && (
              <div className="space-y-2 text-sm">
                ...
                <p className="text-xs text-slate-400">
                  One Gemini call per drill set (teaching only). The correct note
                  name is computed, never guessed by the model.
                </p>
              </div>
            )}
          </section>
      ...
      {/* Labelled stubs — clearly non-functional, never mistaken for bugs */}
      <section className="mt-10">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {STUBS.map((s) => (
            ...
            >
              {s}
              <div className="mt-1 text-xs text-slate-300">Phase 2 — coming soon</div>
            </div>
          ))}
        </div>
      </section>
