...
A lean, **Hermes-native** harness for building agentic software **spec-first**. One person with an idea and one API key can drive a real, production-shaped agent into existence — and a senior engineer opening the result finds a conventional, reviewable stack, not generated mush.
...
3. **Lean harness, not a framework.** `harness/` is engineering *mindfulness* — rules and patterns that keep every session consistent — deliberately Hermes-native and kept small. The product runtime stays provider-agnostic; the harness does not.
...
- A working **baseline agent** in `src/` (FastAPI + LangGraph + SQLite, provider-agnostic LLM — OpenRouter (primary), Anthropic, or Gemini, `transform_text` as the capability slot) — tests pass out of the box
...
- Engineering rules and patterns in `harness/` so every Hermes session is consistent
...
### Step 2 — Open in Hermes

Start a Hermes session with the working directory inside this repo. Hermes discovers `.hermes.md` (the project context) automatically and reads `AGENTS.md` first.
...
```
src/                ← baseline agent (FastAPI + LangGraph + SQLite; OpenRouter/Anthropic/Gemini)
  ...
  llm/              ← LLM client + providers/ (openrouter, anthropic, gemini)
...
harness/
  ...
  agents/           ← agent-builder, spec-writer, code-generator, qa-auditor (run via delegate_task)
AGENTS.md           ← entry point — read first every session
.hermes.md          ← Hermes project context (auto-discovered)
pyproject.toml
...
#   AGENT_OPENROUTER_API_KEY=<your key>   (primary — any model via OpenRouter)
#   or AGENT_ANTHROPIC_API_KEY=<your key>   or   AGENT_GEMINI_API_KEY=<your key>
...
- Never commit on `main` — all work happens on a feature branch with a unique slug, PR'd back to the branch it was cut from
