"""Live DB analyst API — read-only queries over live SQL Server."""
...
from src.domain.live_db_analyst import LiveDBQueryRequest, LiveDBQueryResponse
from src.graph.agent_live_db import build_live_db_graph
...
@router.post("/query")
def query_live_db(req: LiveDBQueryRequest, session: Session = Depends(get_session)) -> dict:
 if not (req.schema_summary or "").strip():
  raise api_error("bad_request", "schema_summary is required for live_db queries.", 400)
...
def get_live_db_run(run_id: str, session: Session = Depends(get_session)) -> dict:
