...
> **Hard gate:** a phase isn't done until it's committed, pushed, and the PR is open/updated — do the git steps the moment slices are VERIFIED, before writing the handoff. A delegated build can end its turn after code is verified but before git runs, so the invoking skill must verify the PR exists on return and finish the git steps itself if they were skipped (see `references/hermes-pitfalls.md` §1).

> **The human gate owns the RUN — the user never launches anything.** Before asking the "does it work?" checklist (one single-select `clarify` per feature — multi-select is not available in Hermes), the root session MUST: (1) launch the server with the explicit project interpreter on a **free** port (retry if busy), (2) smoke-test it live — health + the new endpoint(s) + a real in-browser render with **0 console errors** (the root session can use its own browser), (3) hand the user **ONE live URL** + a one-line "what to click". Only THEN ask the checklist. If the server won't boot, that is a **BLOCKER**, not a question — fix it, don't bounce it to the user. Bouncing an un-run app back as a `clarify` is the failure mode to avoid (see `references/hermes-pitfalls.md` §7).
