...
> **Hard gate — the handoff is NOT done until committed + pushed + PR is open/updated.** Verified code with no commit is an incomplete handoff. In a runtime where a delegated build runs with an iteration/step budget, the delegation can end its turn *after the code is written and verified but before the git steps run* (a "returned at ~95%" finish). Guard against it two ways: (a) treat commit/push/PR as part of the phase, not an epilogue — do it the moment slices are VERIFIED, before writing prose; and (b) the invoking skill (root session) MUST verify the branch is committed, pushed, and the PR exists when the delegation returns, and **finish those git steps itself** if they were skipped. Never present a test-handoff for a phase whose PR does not yet exist.
